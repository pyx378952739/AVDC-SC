...
# ========================================================================获取中文标题（辅助函数）
def getChineseTitle(file_number, config, original_title, appoint_url):
    """尝试从其他数据源获取中文标题"""
    # 检查用户是否配置了中文优先
    try:
        lang_priority = config.get('language', 'priority', fallback='sc,tc,ja')
        priority_list = [x.strip().lower() for x in lang_priority.split(',')]
    except:
        priority_list = ['sc', 'tc', 'ja']
    
    # 如果用户没有配置中文优先，则不尝试获取中文标题
    if priority_list[0] not in ['sc', 'tc']:
        return original_title
    
    # 尝试从 javdb 获取中文标题
    try:
        javdb_result = json.loads(javdb.main(file_number, appoint_url, is_uncensored(file_number)))
        if javdb_result.get('title') and javdb_result['title'] != original_title:
            # javdb 返回的标题通常更短，如果原标题包含 javdb 标题，则使用原标题（日文）
            # 如果 javdb 标题明显不同，则可能是翻译版
            javdb_title = javdb_result['title'].replace(file_number.upper(), '').replace(file_number, '').strip('- ')
            orig_clean = original_title.replace(file_number.upper(), '').replace(file_number, '').strip('- ')
            # 如果 javdb 标题较短且原标题包含 javdb 标题关键字，则保持原样
            if len(javdb_title) < len(orig_clean) * 0.8:
                return original_title
            return javdb_result['title']
    except:
        pass
    
    # 尝试从 jav321 获取
    try:
        jav321_result = json.loads(jav321.main(file_number, appoint_url))
        if jav321_result.get('title') and jav321_result['title'] != original_title:
            return jav321_result['title']
    except:
        pass
...
def getDataFromJSON(file_number, config, mode, appoint_url):  # 从JSON返回元数据
    ...
    if json_data['website'] == 'timeout':
        ...
        return json_data
    # ======================================尝试获取中文标题
    if json_data.get('title') and json_data.get('source') == 'javbus.py':
        # 如果数据来自 javbus，尝试获取中文标题
        chinese_title = getChineseTitle(file_number, config, json_data['title'], appoint_url)
        if chinese_title != json_data['title']:
            json_data['title'] = chinese_title
    ...
    if translate_enabled:
        try:
            ...
            if 'title' in json_data:
                print(f'[+] 翻译完成')
            json_data['title_display'] = json_data.get('title', title)
        except Exception as e:
            print(f'[-] 翻译失败: {e}')
            json_data['title_display'] = title
            json_data['title_translated'] = title
            json_data['title_original'] = title
    else:
        json_data['title_display'] = title
        json_data['title_translated'] = title
        json_data['title_original'] = title
...
def save_config(json_config):
    ...
    with open(config_file, "wt", encoding='UTF-8') as code:
        ...
        print("no_file_move = " + str(json_config.get('no_file_move', 0)), file=code)
        print("# all or mgstage or fc2club or javbus or jav321 or javdb or avsox or xcity or dmm", file=code)
        print("# no_file_move: 0=正常移动文件, 1=仅刮削不移动(在原目录创建metadata文件夹)", file=code)
        print("", file=code)
        print("[language]", file=code)
        print("priority = " + json_config.get('language_priority', 'sc,tc,ja'), file=code)
        print("# sc=简体中文, tc=繁体中文, ja=日文, en=英文", file=code)
        print("", file=code)
        ...
        print("", file=code)
        print("[translate]", file=code)
        print("enabled = " + str(json_config.get('translate_enabled', 'True')), file=code)
        print("# 翻译功能：将获取到的影片信息翻译为简体中文（默认启用）", file=code)
...
def check_pic(path_pic):
    try:
        # 检查文件是否存在且不为空
        if not os.path.exists(path_pic) or os.path.getsize(path_pic) == 0:
            return False
