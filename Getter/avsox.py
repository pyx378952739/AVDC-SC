...
def get_avsox_lang():
    """获取 avsox 使用的语言路径
    avsox 支持: cn (简体中文), ja (日文), en (英文), tw (繁体中文)
    根据用户优先级返回合适的语言代码
    """
...
def getUrl(number):
    lang = get_avsox_lang()
    response = get_html(f'https://avsox.website/{lang}/search/' + number)
...
def main(number, appoint_url=''):
    try:
        ...
        # 确保使用正确的语言版本访问详情页
        lang = get_avsox_lang()
        if '/cn/' in url or '/ja/' in url or '/en/' in url or '/tw/' in url or '/hk/' in url:
            # 替换语言路径
            url = re.sub(r'/(cn|ja|en|tw|hk)/', f'/{lang}/', url)
        web = get_html(url)
