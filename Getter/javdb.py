...
import json
from Function.getHtml import get_html_javdb, get_language_priority
from Function.getHtml import post_html


# ========================================================================获取 javdb 语言参数
def get_javdb_locale():
    """获取 javdb 使用的 locale 参数
    javdb 支持: zh (繁体中文), cn (简体中文), ja (日文), en (英文)
    根据用户优先级返回合适的语言代码
    """
    priority = get_language_priority()
    # 语言映射: sc->cn (简体中文), tc->zh (繁体中文), ja->ja (日文), en->en (英文)
    lang_map = {
        'sc': 'cn',
        'tc': 'zh', 
        'ja': 'ja',
...
def main(number, appoint_url, isuncensored=False):
    try:
        if appoint_url == '':
            # ========================================================================搜索番号
            locale = get_javdb_locale()
            htmlcode = get_html_javdb(f'https://javdb.com/search?q={number}&f=all&locale={locale}').replace(u'\xa0', u' ')
        ...
        # ========================================================================请求、判断结果
        locale = get_javdb_locale()
        html_info = get_html_javdb(result_url + f'?locale={locale}').replace(u'\xa0', u' ')
        if str(html_info) == 'ProxyError':
            raise TimeoutError
        # ========================================================================获取评分、简介
        imagecut = 1
        outline = ''
        if isuncensored and (re.match(r'^\d{4,}', number) or re.match(r'n\d{4}', number)):  # 无码，收集封面、评分
            imagecut = 0
            ...
            for count in range(1, counts + 1):  # 遍历搜索结果，找到需要的番号
                ...
                if not date_get:
                    date_get = html.xpath(
                        '//div[@id=\'videos\']/div[@class=\'grid columns\']/div[@class=\'grid-item column horz-cover\'][' + str(
                            count) + ']/a[@class=\'box\']/div[@class=\'meta\']/text()')[0]
                if re.search(r'\d{4}-\d{1,2}-\d{1,2}', date_get):
                    date_get = re.findall(r'\d{4}-\d{1,2}-\d{1,2}', date_get)[0]
                elif re.search(r'\d{1,2}/\d{1,2}/\d{4}', date_get):
                    date_get_tmp = date_get
                    date_get = re.findall(r'\d{1,2}/\d{1,2}', date_get_tmp)[0]
                    year_get = re.findall(r'\d{4}', date_get_tmp)[0]
