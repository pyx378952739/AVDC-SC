...
import json
from Function.getHtml import post_html, get_language_priority


# ========================================================================获取语言优先级（用于请求头）
def get_jav321_lang_header():
    """获取 jav321 使用的 Accept-Language 头
    jav321 主要支持日文，但会尝试按用户优先级设置
    """
    priority = get_language_priority()
    # 构建 Accept-Language 头
    lang_parts = []
    q = 0.9
    for lang in priority[:3]:  # 最多取前3个
        if lang == 'sc':
            lang_parts.append(f'zh-CN;q={q}')
        elif lang == 'tc':
            lang_parts.append(f'zh-TW;q={q}')
        elif lang == 'ja':
            lang_parts.append(f'ja;q={q}')
        elif lang == 'en':
            lang_parts.append(f'en;q={q}')
        q = round(q - 0.1, 1)
    return ', '.join(lang_parts) if lang_parts else 'ja, zh-CN;q=0.9'
